---
name: git-operations
description: 執行 XDDPMAPP 專案的 Git 檢查、分支與 commit 操作，並遵循 docs/git_conventions.md 的 Git 命名規範；不處理未明確授權的破壞性或遠端操作。
metadata:
...
本 skill 用於 XDDPMAPP 專案中需要檢查狀態、建立或檢查分支、整理 commit、檢視 diff，或執行其他 Git 操作的工作。專案 Git 規範的唯一來源是 [`docs/git_conventions.md`](../../../docs/git_conventions.md)；每次需要命名、commit 或分支操作時，先讀取該文件，不要自行創造另一套規則。

## 工作邊界

- 先讀取適用的 `AGENTS.md` 與 `docs/git_conventions.md`，再執行會改變工作樹或歷史的操作。
...
任何需要建立或檢查分支、撰寫或檢查 commit message 的工作，都必須先讀取 [`docs/git_conventions.md`](../../../docs/git_conventions.md) 的 `# Git 命名規範`，並依該文件底下的 `## Commit message` 或 `## Branch` 執行。不要在本 skill 重述、複製或自行推導 Git 命名規則；`git_conventions.md` 是唯一規範來源。
