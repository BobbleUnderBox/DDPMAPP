...
以「流程實體」串起操作：流程保存選定模型、適用範例、推論參數與 random seed 設定，對外最多對應一個 run 與一個 analyse。所屬 run 完成後才可建立 analyse；兩者皆完成後，可彙整該流程的報表。修改、重新執行與刪除的對外規則依 [API 定義](api.md)。
...
run 與 analyse 的 `status` 及其意義統一由 [API 定義](api.md#任務狀態) 維護。協調程序同步狀態與進度，前端透過 FastAPI 查詢。
