...
找到 **五款具有官方任務 checkpoint、可直接下載**的 diffusion models：DermoSegDiff、cDAL、CCDM、THOR 與 DiffAtlas。因此不必把「寄信申請權重」當成主清單的必要補位；需要更多候選時，AnoDDPM 可依作者指示聯絡索取。
...
官方 testing scripts 使用 64×64×64、6 channels、3D U-Net、batch 1 與 300 timesteps。依輸入尺寸判斷，T4 16 GB 應可執行單一 crop，但這是五款中最需要實機驗證的一款；若 OOM，需嘗試 autocast／FP16，且先確認模型的 normalization layers 在 half precision 下穩定。
...
5. **DiffAtlas**：3D 多結構展示效果佳，卻是顯存、延遲與前處理風險最高者。
...
- DermoSegDiff 與 CCDM 程式為 MIT；DiffAtlas 為 Apache-2.0；cDAL 明確限制非商業研究／評估；THOR 未附明確程式授權。
